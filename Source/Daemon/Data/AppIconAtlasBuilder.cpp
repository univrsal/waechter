...
bool WAppIconAtlasBuilder::GetAtlasData(WAppIconAtlasData& outData, std::vector<std::string> const& BinaryNames, std::size_t AtlasSize)
{
	...
	for (auto const& BinaryName : BinaryNames)
	{
		...
		for (std::size_t i = 0; i < NumIcons; ++i)
		{
			...
			{
				spdlog::error("Icon '{}' ({}/{})) does not fit in the atlas.", IconPath, i, NumIcons);
				return false;
			}
		}
