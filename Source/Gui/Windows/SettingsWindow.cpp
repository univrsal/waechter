...
void WSettingsWindow::DrawClientSettings()
{
	...
	ImGui::Checkbox(TR("settings.socket.ws_allow_self_sign"), &WSettings::GetInstance().bAllowSelfSignedCertificates);
	ImGui::Checkbox(TR("settings.socket.ws_skip_cert_check"), &WSettings::GetInstance().bSkipCertificateHostnameCheck);
#endif
	...
	if (ImGui::Checkbox(TR("settings.ui.reduce_fps_inactive"), &WSettings::GetInstance().bReduceFrameRateWhenInactive))
	{
		if (WSettings::GetInstance().bUseDarkTheme)
		{
			ImGui::StyleColorsDark();
...
void WSettingsWindow::Draw()
{
	...
	{
		if (ImGui::BeginTabBar("##SettingsWindow"))
		{
			if (ImGui::BeginTabItem(TR("settings.client")))
			{
				DrawClientSettings();
				ImGui::EndTabItem();
			}
			if (ImGui::BeginTabItem(TR("settings.daemon")))
			{
				WMainWindow::Get().GetSetupWindow().DrawDaemonSettings();
				ImGui::EndTabItem();
			}
		}
		ImGui::EndTabBar();
