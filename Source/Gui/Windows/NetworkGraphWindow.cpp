...
void WNetworkGraphWindow::Draw()
{
	...
	if (ImGui::Begin(TR("network_activity.title"), nullptr, ImGuiWindowFlags_None))
	{
		// History duration dropdown
		static char const*  HistoryOptions[] = { "1 min", "5 min", "15 min" };

		ImGui::SetNextItemWidth(100);
		if (ImGui::Combo(TR("network_activity.duration"), &WSettings::GetInstance().NetworkGraphHistorySetting,
				HistoryOptions, IM_ARRAYSIZE(HistoryOptions)))
		{
		...
		ImGui::SetNextItemWidth(100);
		ImGui::SliderFloat(
			TR("network_activity.line_width"), &WSettings::GetInstance().NetworkGraphLineWidth, 0.5f, 5.0f, "%.1f");
		...
		if (ImPlot::BeginPlot("##Scrolling", ImVec2(-1, -1)))
		{
			...
			ImPlot::PlotStairs(TR("generic.upload"), &UploadInverted[0].x, &UploadInverted[0].y, UploadInverted.size(),
				0, 0, 2 * sizeof(float));
			...
			ImPlot::PlotStairs(TR("generic.download"), &DownloadInverted[0].x, &DownloadInverted[0].y,
				DownloadInverted.size(), 0, 0, 2 * sizeof(float));
			ImPlot::EndPlot();
		}
	}
	ImGui::End();
